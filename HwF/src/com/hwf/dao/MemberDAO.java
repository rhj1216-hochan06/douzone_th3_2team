...
public class MemberDAO {
	...
	//AdminUserSelect
	public List<MemberDTO> AdminUserSelect() { //null처리도 해주기 위해서 위의 주석문장을 try~catch로 감싸줌
		try {
			sqlSession = sqlSessionFactory.openSession(); //어플리케이션과 DB 통로 역할
			
			return sqlSession.selectList("dao.AdminUserSelect"); //member-mapper.xml에서 namespace.id와 일치시켜주어야 함
